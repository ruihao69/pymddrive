...
namespace rhbi {
// ----------------------------------------------------------------------------
// Tensor / Matrix conversion template functions
// Reference: DavidAce on StackOverflow
...
// Converts an Eigen::Matrix (or expression) to Eigen::Tensor
// with dimensions as variodic arguments
...
//---
template <typename T>
using TensorRank3Type = Eigen::Tensor<T, 3>;

template <typename T>
const Eigen::Array<T, Eigen::Dynamic, 1> rank3_tensor_view(
    const TensorRank3Type<T> &tensor,
    Eigen::Index ii_electronic,
    Eigen::Index jj_electronic) {
  const Eigen::Tensor<T, 1> view = tensor.chip(ii_electronic, 0).chip(jj_electronic, 0);
  return Eigen::Map<const Eigen::Array<T, Eigen::Dynamic, 1>>(view.data(), view.size());
}
}  // namespace rhbi