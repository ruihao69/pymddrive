...
def boltzmann_sampling(
    ...
    R_ensemble = [np.random.normal(0, sigma_R_alpha) for _ in range(n_ensemble)]
...
def wigner_sampling(
    ...
    R_ensemble = [np.random.normal(0.0, sigma_R) for _ in range(n_enemble)]
    P_enemble = [np.random.normal(0.0, sigma_P) for _ in range(n_enemble)]
    return R_ensemble, P_enemble
