...
from pymddrive.models.tully_w_pulse import TullyOnePulseTwo
...
def run_tullyone_pulsetwo(r0, p0, Omega, tau, mass=2000, solver='Ehrenfest', method='vv_rk4'):
    A = 0.01
    B = 1.6
    C = 0.005
    D = 1.0
    ...
    pulse = MorletReal(A=1.0, t0=_delay, tau=tau, Omega=Omega, phi=0)
    
    # model = TullyOnePulseOne(A, B, pulse=pulse)
    model = TullyOnePulseTwo(A, B, C, D, pulse=pulse)
...
def main(sim_signature: str, n_samples: int, Omega, tau, p_bounds: tuple=(0.5, 35.0)):
    ...
    res_gen = runner.run(run_tullyone_pulsetwo, accumulate_output, sim_signature)
...
if __name__ == "__main__":
    ...
    desc = "The parser for TullyOne with Pulse Two"
...
#     output, pulse = run_tullyone_pulsetwo(r0=-10, p0=10, Omega=0.1, tau=100)
#     times = output['time']
#     R = output['states']['R']
#     P = output['states']['P']
